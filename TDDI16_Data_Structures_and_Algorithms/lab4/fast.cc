  ...
  for (int i{0}; i < N; ++i)
    {
      ...
      for (int j{i + 1}; j < N; ++j)
	{
	  tempPoints.push_back(points.at(j));
	}
      PolarSorter ps(points.at(i));
      stable_sort(tempPoints.begin(), tempPoints.end(), ps);
	  ...
	  for (unsigned int j{0}; j < tempPoints.size()-2; ++j)
	    {
	      ...
	      if (points.at(i).slopeTo(tempPoints.at(j)) == points.at(i).slopeTo(tempPoints.at(j + 1)) &&
		  points.at(i).slopeTo(tempPoints.at(j)) == points.at(i).slopeTo(tempPoints.at(j + 2)) )
		{

		  render_line(screen, points.at(i), tempPoints.at(j + 2));
		}
	    }
	}
    }
