...
public:
  ...
  void toss(const Comparable & x)
  {
    ...
    // Add x to heap and add one to currentSize
    array[++currentSize] = x;

    // Set orderOK to false
    orderOK = false;

    /* What to be done:
     * Add lazy insert to toss, set orderOK to false
     * Change in findMin and deleteMin so we fix the heap with fixHeap? before returning min
     */
  }
  ...
  void deleteMin(Comparable & minItem)
  {
    ...
    // No need to check orderOK here, thats done in findMin()
    minItem = findMin();
